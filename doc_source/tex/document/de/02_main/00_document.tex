...
den beiden Datensets unterscheiden kann \cite[p.~4-5]{8253599}. Es gilt $p_{data} = p_{g}$. Wichtig hierbei zu erwähnen ist
die Tatsache, dass der Diskriminator mit dem Generator gekoppelt zu sein scheint. Soll heissen, dass wenn der Diskriminator im Schnitt für
die Samples aus dem originalen Datenraum den Wert $x$ angibt, er für Samples aus dem generierten Datenraum $1 - x$ als Resultat liefert.
...
sowie der erwarteten Wahrscheinlichkeiten von $D$ für die originalen Samples. Für den Diskriminator entspricht $P(x) = p_{data}(x) = 1$ für alle Wahrscheinlichkeiten,
...
\begin{align}
    H(p_{data},D) = - \sum_{x \sim p_{data}} (p_{data}(x) \cdot log(D(x)) + (1 - p_{data}(x)) \cdot log(1 - D(x))) \label{eq:8}\\
...
In diesem Fall ist $P(x) = p_{g}(x) = 0$ für alle erwarteten Wahrscheinlichkeiten (das generierte Sample gehört nicht zum
originalen Datenset), $Q(x) = D(x)$ und $x$ stammt aus dem generierten Datenset.
\begin{align}
    H(p_{g}(x),D) = - \sum_{x \sim p_{g}} (p_{g}(x) \cdot log(D(x)) + (1 - p_{g}(x)) \cdot log(1 - D(x)))\\
    H(0,D) = - \sum_{x \sim p_{g}} log(1 - D(x)) \label{eq:9}
\end{align}
Wenn nun in Betracht gezogen wird, dass $x$ nicht der Wahrscheinlichkeitsverteilung $p_g$, sondern der Noise-Verteilung $p_z$ folgt und $x = G(z)$ gilt,
...
\para
Wie Eingangs erwähnt, bildet $G$ ein Mapper von Noise-Samples $z$ nach Samples $x ~ p_{data}$. Weiterhin ist nach
