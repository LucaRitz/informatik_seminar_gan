...
\end{align}
Diese Value-Funktion definiert die umgekehrte Fehlerfunktion von $D$. Nachfolgend werden einige Gedanken des Autors dazu aufgeführt.\\
...
wird eine Loss-Funktion sowohl für den Generator, wie auch den Discriminator angegeben. Die Loss-Funktionen werden im
nachfolgenden einzeln besprochen. Zuerst wird sich dem Generator gewidmet.
\begin{lstlisting}
...
sowie der Wahrheit (alle Wahrscheinlichkeiten sind 1). Dies bedeutet, der Fehler für den Generator ist $0$, wenn der Diskriminator
...
\begin{lstlisting}
    def discriminator_loss(real_output, fake_output):
        ...
        return total_loss
\end{lstlisting}
An dieser Stelle werden die einzelnen Zeilen der Gleichung \ref{eq:1} zugewiesen. Dazu muss bekannt sein, dass der
Diskriminator ein binäres Problem beschreibt. Weiterhin ist die Gleichung \ref{eq:1} der Kreuzentropie entnommen. Diese
...
\end{align}
Die erste Zeile berechnet die Kreuzentropie zwischen den berechneten Wahrscheinlichkeiten der originalen Samples
sowie der erwarteten Wahrscheinlichkeiten. Für den Diskriminator entspricht $P(x) = 1$ für alle Wahrscheinlichkeiten,
wobei $Q(x) = D(x)$ gilt und $x$ aus $p_{data}$ stammt. Eingesetzt in Formel \ref{eq:7} ergibt dies \ref{eq:8}
\begin{align}
    H(P,Q) = - \sum_{x \in p_{data}} (P(x) \cdot log(D(x)) + (1 - P(x)) \cdot log(1 - D(x))) \label{eq:8}\\
    H(P,Q) = - \sum_{x \in p_{data}} log(D(x)) \label{eq:11}
\end{align}
Die darauffolgende Zeile entspricht dem zweiten Summand der Gleichung\ref{eq:1}.
...
\end{align}
Zusammenaddiert ergeben die Formeln \ref{eq:11} und \ref{eq:10} fast den Loss-Wert der Formel \ref{eq:1} und damit die Minimax-Value-Funktion,
welche nach den Formeln \ref{eq:11} und \ref{eq:10} eigentlich von $D$ minimiert werden will.
\begin{align}
    V(D,G) = - \sum_{x \in p_{data}} log(D(x)) + - \sum_{z \in p_{z}} log(1 - D(G(z)))
\end{align}
Da die negativen Vorzeichen jeweils weggelassen werden, wird die Richtung der Optimierung umgedreht,
was auch erklärt, wieso die Loss-Funktion nach $G$ und nicht mehr nach $D$ minimiert wird.
TODO: Fehlt ein arithmetisches Mittel, Median? Was ist $E$?
